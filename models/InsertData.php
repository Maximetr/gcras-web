...
class InsertData {
    ...
    public static function CheckMinute($rows) {
        ...
        $regular = '~((\\s*[0-9]{1,5})|([0-9]{6})){2}[0-9]{2}((\\s[0-9]|1[0-2])|(0[0-9]|1[0-2]))((\\s[0-9])|([0-9]{2}))(D|I|H|X|Y|Z|E|F)((\\s[0-9])|([0-9]{2}))[A-Z]{3}.(0|9|8|\\s)(P|D|\\s)\\s{7}((\\s|9|-)(\\s*-?[0-9]{1,5})){61}~';        $lineNumber = 1;
    ...
    public static function CheckHourly($rows) {
        ...
        $regular = "~([A-Z]{3})([0-9]{2})((\\s[0-9]|1[0-2])|(0[0-9]|1[0-2]))([D|H|Z|F|X|Y])((\\s|[0-3])[0-9])(\\s{2})(.{2})((\\s{2}|18|19|20)|([Q|D|\\s]{1})([\\s|8]{1}))([\\s*-?0-9]{4})([\\s|9|-][\\s*-?0-9]{3}){25}~";
        $lineNumber = 1;
        ...
        foreach ($rows as $row) {
            ...
            for ($i = 34, $j = strlen($row); $i < $j;) {
                if ($row[$i] == '9' && $i <> 394) {
                    ...
                    $minutevalues[] = trim(substr($row,$i+1,5),' ');
                    }
                if ($i == 394 && $row[$i] <> '9') {
                    $hourvalues = trim(substr($row,395,5), ' ');
        ...
        foreach ($rows as $row) {
            ...
            for ($i = 20, $j = strlen($row); $i < $j;) {
                if ($row[$i] == '9' && $i <> 116) {
                    ...
                    $hourvalues[] = trim(substr($row,$i+1,3),' ');
                    }   elseif ($row[$i] == '-' && $i<>116) {
                        $hourvalues[] = '-'.trim(substr($row,$i+1,3),' ');
                    }
                if ($i == 116 && $row[$i] <> '9') {
                    $dayvalues = trim(substr($row,117,3), ' ');
            ...
            `HourSet1`, `HourSet2`, `HourSet3`, `HourSet4`, `HourSet5`, `HourSet6`, `HourSet7`, `HourSet8`, `HourSet9`, `HourSet10`, `HourSet11`, `HourSet12`, `HourSet13`, `HourSet14`, `HourSet15`, `HourSet16`, `HourSet17`, `HourSet18`, `HourSet19`, `HourSet20`, `HourSet21`, `HourSet22`, `HourSet23`, `HourSet24`, `DailySet`) 
            ...
            '$hourvalues[15]', '$hourvalues[16]', '$hourvalues[17]', '$hourvalues[18]', '$hourvalues[19]', '$hourvalues[20]', '$hourvalues[21]', '$hourvalues[22]', '$hourvalues[23]', '$dayvalues')";
